...
@observer
export default class CommonplaceView extends React.Component {
  constructor(props: any) {
    super(props);
    // assume annotations are sorted
    let location: number = 0

    if (this.props.books[0].annotations[0]) {
      location = this.props.books[0].annotations[0].location
    }
  ...
  addAllAnnotationsToEditor() {
    const { books, notesStore } = this.props
  ...
  render() {
    ...
    return (
      <div className="flex w-100 h-100">
        <button className="btn f6 mt4 fr">
          Test button
        </button>
      </div>
    );
