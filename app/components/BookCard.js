...
export default class BookCard extends React.Component {
  ...
  render() {
    ...
    if (book.highlightsUpdatedAt) {
      footer = (<p className="gray ma0"><i className="fa fa-comment" aria-hidden="true" /> {book.annotations ? book.annotations.length : 0} highlights</p>)
    } else {
    ...
    return (
      <div key={book.asin} className="w-20-l w-third-m w-100 pa3-ns pb3 f5 v-top flex">
        <div className="bg-white flex flex-column shadow-1 w-100">
          <Link className="no-underline" to={`/book/${book.asin}`} >
